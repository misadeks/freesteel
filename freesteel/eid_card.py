...
class CardFile:
	...
	CERT_STANDARD = (0x0F, 0x16)  # Public X.509 certificate for standard signing
...
class EidCard:
	...
	def __get_fields(self, data):
		...
		for name, value in fields.items():
			#added on 02.02.2022. misadeks
			fields[name] = bytes.fromhex("".join([format(x, "02x") for x in value])).decode('utf-8')
			#fields[name] = scard_util.toASCIIString(value)
			#print(scard_util.toASCIIString(value))
	...
	def get_photo(self):
		...
		#fixed this 02.02.2022. misadeks
	...
	def get_certificate(self):
		...
		data = self._card.read_file(CardFile.CERT_STANDARD)[1]
		data = bytes(data[4:])
		return data

	def get_file(self, file):
		data = self._card.read_file(file)[1]
		data = bytes(data[4:])
		return data
